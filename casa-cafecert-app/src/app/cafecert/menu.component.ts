...
@Component({ selector: 'app-menu-2', templateUrl: 'menu.component.html' })
export class MenuComponent implements OnInit {
    ...
    ngOnInit() {
        this.rutaActiva.queryParams.subscribe(params => {
            ...
            if (this.sessionid === null) {
                ...
                this.menu = JSON.parse('[{"menuid":"0","item":"No existe sesión id , el menú principal ha sido remplazado por este enlase, usted debe hacer click para volver a la pagina login","url":' + '"' + `${process.env.NEXT}` + '"' + '}]');
            } else {
        ...
        this.http.post<any>(`${process.env.USERBUSSINESS}`+`/cafecert/findsession/`, {
            ...
            .subscribe(data => {
                if (data['status'] === `Internal Server Error`) {
                    this.alertService.error(data['message']);
                    this.router.navigate(['/cafecert/home/']);
                    this.menu = JSON.parse('[{"menuid":"0","item":"Ocurrió un error, el menú principal ha sido remplazado por este enlase debido, usted debe hacer click para volver a la pagina login","url":' + '"' + `${process.env.NEXT}` + '"' + '}]');
                } else {
                    if (data['expired'] === `-1`) {
                        this.alertService.error('Sesión ha expirado, debe volver a loguearse en el sistema.');
                        this.menu = JSON.parse('[{"menuid":"0","item":"Sesión ha expirado, el menú principal ha sido remplazado por este enlase debido, usted debe hacer click para volver a la pagina login","url":' + '"' + `${process.env.NEXT}` + '"' + '}]');
                    } else {
                        if (data['role'] === null) {
                            this.mostrarboton = false;
                            this.menu = JSON.parse('[{"menuid":"0","item":"No existe rol de usuario registrado en el sistema, el menú principal ha sido remplazado por este enlase, usted debe hacer click para volver a la pagina login","url":' + '"' + `${process.env.NEXT}` + '"' + '}]');
                        } else {
    ...
    chooseMenu(url: string) {
       // window.open(url + "?param=" + this.sessionid, '_blank');
