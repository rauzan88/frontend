                ...
                if (data['status'] === `Internal Server Error`) {
                    ...
                    if (data['expired'] === `-1`) {
                        ...
                        if (data['role'] === null) {
                            ...
                            this.listasService.obtenerArchivos({ codrequest : this.codrequest, frontend: `${process.env.FRONTEND}`, next: `${process.env.NEXT}`, sessionid: this.sessionid, textrole: data[`role`], textuserdni: data[`dni`] })
                                .subscribe(data => {
                                    this.lista = data;
                                });
                        }
