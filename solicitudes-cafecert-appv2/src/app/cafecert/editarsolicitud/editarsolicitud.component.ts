...
export class EditarSolicitudComponent implements OnInit {
    ...
    listaTiposCafe: any;
    listaMarcas: Array<{ certificateProductId: string, productId: string, certificateId: string, productTypeId: string, programTypeId: string, txApproved: string, productTypeName: string, programTypeName: string, txPlant: string, txName: string, txReference: string, txPresentation: string, txCoffeeOwner: string, txCountry: string, txActive: string, txCreationUser: string, dtCreation: string }> = [];
    listaMarcasDel: Array<{ certificateProductId: string, productId: string, certificateId: string, productTypeId: string, programTypeId: string, txApproved: string, productTypeName: string, programTypeName: string, txPlant: string, txName: string, txReference: string, txPresentation: string, txCoffeeOwner: string, txCountry: string, txActive: string, txCreationUser: string, dtCreation: string }> = [];
    ...
    public validarSesion(): void {
        ...
        this.requestcode = JSON.parse(localStorage.getItem("request")).cdRequest;
                ...
                if (data['status'] === `Internal Server Error`) {
                    ...
                    if (data['expired'] === `-1`) {
                        ...
                        if (data['role'] === null) {
                            ...
                            */
                            this.listasService.productosByRequest({ "frontend": `${process.env.FRONTEND}`, "next": `${process.env.NEXT}`, "sessionid": this.sessionid, "userrole": this.role, "request": this.requestcode })
                                .subscribe(data => {
    ...
    cargarListasFormulario(role: any): void {
        // CDREQUEST               
        console.log(this.objSolicitud = JSON.parse(localStorage.getItem("request")));
    ...
    cargarPorDefectoListas(): void {
        ...
        console.log("------>",this.objRepresentante = JSON.parse('[{"txFirstName":"", "txLastName":"", "txAddress":"", "txEmail":"", "txMobile":""}]') || []);
    ...
    consultarInfoRepresentante(cdclient: any): void {
        ...
        this.listasService.consultarRepresentanteCliente({ rol: this.role, sesionid: this.sessionid, frontend: `${process.env.FRONTEND}`, next: `${process.env.NEXT}`, cdClient: cdclient })
            .subscribe(data => {
                ...
                console.log("---> cargarControlesRepresentante :",this.cargarControlesRepresentante(this.objRepresentante));
            });
    }
    ...
    public isFormValid(): boolean {
        if (this.f.itemPaises.value == '' || this.f.rbtipocertificacion.value == '' ||
            ...
            this.f.direccioncorrespondencia.value == '' || //this.f.numerofijo.value == '' || this.f.celular.value == '' ||
            this.f.email.value == '' ||
    ...
    crearMarcas(): void {
        ...
        for (let marca of this.listaMarcas) {
            this.crearMarca(marca.txName, marca.txReference, marca.productTypeId, "0", marca.txPresentation, marca.txPlant, marca.txCoffeeOwner, marca.txCountry);
        }
    }

    procesarMarcas(): void {
        let eliminatodo = true;
        if (this.listaMarcasDel.length > 0) {
            //recorre lista marcadas para borrar y se corre el proceso para borrar
            for (let delitem of this.listaMarcasDel) {
                this.listasService.eliminarProdCertiRelation({
                    "certificateProductId": delitem.certificateProductId,
                    "productId": delitem.productId,
                    "certificateId": delitem.certificateId,
                    "requestId": this.requestcode,
                    "productTypeId": delitem.productTypeId,
                    "programTypeId": delitem.programTypeId,
                    "txApproved": delitem.txApproved,
                    "frontend": `${process.env.FRONTEND}`,
                    "next": `${process.env.NEXT}`,
                    "sessionid": this.sessionid,
                    "userrole": this.dni
        ...
        console.log("eliminatodo: " + eliminatodo);
        if (eliminatodo) {
            //si no hubo errores en el borrado de marcar se crear las nuevas marcas
            for (let item of this.listaMarcas) {
                //console.log("marca.productId: " + item.productId);
                if (item.productId === "0") {
            ...
            this.alertService.error('Ocurrio un error a eliminar');
        }
    ...
    public onClickAdd(): void {
        ...
        if (this.f.nombremarca.value.toString() != "" && this.f.tipoproducto.value.toString() != "") {
            ...
            //Array<{ certificateProductId: string, productId: string, certificateId: string, productTypeId: string, programTypeId: string, txApproved: string, productTypeName: string, programTypeName: string, txPlant: string, txName: string, txReference: string, txPresentation: string, txCoffeeOwner:string, txCountry:string, txActive:string, txCreationUser:string, dtCreation:string}> = [];
            let trademark = {
                certificateProductId: "0",
                productId: "0",
                certificateId: "0",
                productTypeId: this.f.tipoproducto.value,
                programTypeId: this.f.tipoprograma.value,
                txApproved: "0",
                productTypeName: this.buscarTipoProducto(this.f.tipoproducto.value).txDescription,
                programTypeName: this.buscarTipoPrograma(this.f.tipoprograma.value).txDescription + " " + this.buscarTipoPrograma(this.f.tipoprograma.value).txRegion,
            ...
            console.log((this.f.tiposcafe.value).txDescription);
    ...
    onChangeQuestionContact(event): void {
