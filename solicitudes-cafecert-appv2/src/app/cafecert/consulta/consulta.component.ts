...
export class ConsultaComponent implements OnInit {
    ...
    listaTiposCafe: any;
    listaMarcas: Array<{ txName: string, txReference: string, cdProductType: string, produTypeName: string, cdProgramType: string, txtipoprograma: string, txCoffeeOwner: string, txCountry: string, txPresentation: string, cdPlant: string, plantName: string}> = [];
                ...
                if (data['status'] === `Internal Server Error`) {
                    ...
                    if (data['expired'] === `-1`) {
                        ...
                        if (data['role'] === null) {
                            ...
                            this.listasService.productosByRequest({ "frontend": `${process.env.FRONTEND}`, "next": `${process.env.NEXT}`, "sessionid": this.sessionid, "request": JSON.parse(localStorage.getItem("request")).cdRequest  })
                                .subscribe(data => {
    ...
    cargarControlesCliente(cliente: any): void {
        ...
        console.log("cliente.txNit: "+ cliente.txNit);
        console.log("cliente.txName: "+ cliente.txName);
        console.log("cliente.txAddress: "+ cliente.txAddress);
        ...
        if(cliente.txAddress === null || cliente.txMailAddress === null){
            mismaDireccion = false;  
        }else{
            mismaDireccion = cliente.txAddress === cliente.txMailAddress;
        }
    ...
    public onClickAdd(): void {
        ...
        if (this.f.nombremarca.value.toString() != "" && this.f.referencia.value.toString() != "" && this.f.tipoproducto.value.toString() != "" && this.f.duenyocafe.value.toString() != "" && this.f.country.value.toString() != "" && this.f.presentproducto.value.toString() != "" && this.f.presentplant.value.toString() != "" && this.f.tipoprograma.value.toString() != "") {
            ...
            let trademark = {
                ...
                cdProductType: this.f.tipoproducto.value,
                //txDescription: this.buscarTipoCafe(this.f.tiposcafe.value).txDescription,
                produTypeName: this.buscarTipoProducto(this.f.tipoproducto.value).txDescription,
                cdProgramType: this.f.tipoprograma.value,
                txtipoprograma: this.buscarTipoPrograma(this.f.tipoprograma.value).txDescription + " " + this.buscarTipoPrograma(this.f.tipoprograma.value).txRegion,
                ...
                plantName: this.buscarPlanta(this.f.presentplant.value).txDescription
            };
            let trademarkObj = this.listaMarcas.filter(q => q.txName == trademark.txName
                && q.txReference == trademark.txReference
                && q.cdProductType == trademark.cdProductType);
